...
let mk_this_eq_term (base_ty : basety) (t : term) : term =
  let eq = match base_ty with TInt -> EqualInt | TBool -> EqualBool in
...
let mk_binop_type (b : binop) : ty =
  ...
  let ty_arg, ty_result =
    ...
    | EqualInt | NotEqualInt | Greater | GreaterEqual | Less | LessEqual ->
        (TInt, TBool)
  in
...
let mk_unop_type (u : unop) : ty =
  ...
  let ty_arg, ty_result =
    match u with Negate -> (TInt, TInt) | Not -> (TBool, TBool)
  in
