...
let isVariable x
    = Char.is_alpha x || Char.is_digit x || Char.equal x '_'

let span f xs =
    Sequence.take_while ~f xs, Sequence.drop_while ~f xs

let skipSpaces str = Sequence.drop_while ~f:Char.is_whitespace str

let seqToString (str: char seq) = str |> Sequence.to_list |> String.of_char_list
...
let parsePayload str : payload * char seq =
    ...
    | '(' ->
        ...
        if not (Sequence.is_empty rest') then eprintf "LeftOver Payloads %s\n" (seqToString rest');
        let rest =
            match Sequence.hd_exn rest with
            | ')' -> Sequence.tl_eagerly_exn rest
...
let fixAssertionDiscrepancy assertions =
    let assertions = String.substr_replace_all assertions ~pattern:"True" ~with_:"true" in
    let assertions = String.substr_replace_all assertions ~pattern:"False" ~with_:"false" in
...
let parseOldDotLabel (str: string) =
    ...
    if not (Sequence.is_empty str) then eprintf "Unexpected %s\n" (seqToString str);
...
let parseRecVarEntry (str: char seq) =
    ...
    | Some '<' ->
        ...
        let rec aux str acc =
            ...
            match Sequence.hd rest with
            | Some '>' ->
                let acc = if Sequence.is_empty expr then acc else (parseSingle expr) :: acc in
                let assertions = Sequence.tl_eagerly_exn rest |> skipSpaces |> seqToString |> fixAssertionDiscrepancy in
                List.rev acc, assertions
            | Some ',' -> aux (Sequence.tl_eagerly_exn rest) ((parseSingle expr) :: acc)
            | _ -> failwith "Unexpected recursion expression"
        in
        aux (Sequence.tl_eagerly_exn str) []
    | _ -> failwith "invalid recursion variable list, missing '<'"
